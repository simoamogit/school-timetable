...
function ClockWidget() {
  ...
  return (
    <div style={{ position: 'fixed', bottom: 20, right: 20, zIndex: 30,
      ...
      borderRadius: 'var(--radius)', padding: '10px 14px', textAlign: 'right',
    ...
    <>
      ...
      {visible && rect && createPortal(
        <div style={{ position: 'fixed',
          ...
          background: 'var(--card)', border: '1px solid var(--border)',
          borderRadius: 6, padding: '10px 12px', width: 240, zIndex: 9999,
          boxShadow: '0 8px 20px rgba(0,0,0,0.2)', pointerEvents: 'none' }}>
...
function TimetableCell({ day, hour, slot, cellNotes, cellSubs, isLocked, isDragOver, isDragging,
  ...
  return (
    <div
      ...
      onClick={onClick}
      className={`${isDragOver ? 'cell-drag-over' : ''} ${isDragging ? 'cell-dragging' : ''} ${isFree ? 'cell-free' : ''}`}
      style={{
        ...
        borderRadius: 4, padding: '7px 6px', cursor: 'pointer', minHeight: 76,
        display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center',
        gap: 4, position: 'relative', transition: 'opacity 0.1s',
        userSelect: 'none'
      }}
    ...
    >
      {isFree && !hasSub ? (
        <span style={{ fontSize: 10, color: 'var(--free)', fontFamily: 'var(--mono)',
        ...
        <>
          <span style={{ fontSize: 12, fontWeight: 600, color: 'white', lineHeight: 1.2,
            textAlign: 'center', maxWidth: '100%', overflow: 'hidden',
            textOverflow: 'ellipsis', padding: '0 4px', textShadow: '0 1px 2px rgba(0,0,0,0.3)' }}>
            {slot.subject}
          </span>
          {cellNotes.length > 0 && <div onClick={e => e.stopPropagation()}><NoteTooltip notes={cellNotes} /></div>}
        </>
...
function SettingsPanel({ onClose, onReset, onExport, onImport, isLocked, onToggleLock,
  ...
  const actionLabel = (action, details) => {
    switch (action) {
      ...
      case 'slot_free': return `${details.day} ora ${details.hour} · Ora libera (era: ${details.subject || 'vuoto'})`;
  ...
  const settingsTabs = [
    ...
    { id: 'share', label: 'Condivisione' },
    { id: 'hours', label: `Ore nascoste${hiddenHours.length ? ` (${hiddenHours.length})` : ''}` },
    { id: 'changelog', label: 'Cronologia' },
  ];
        ...
        {tab === 'general' && (
          <div>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '12px 0',
              ...
              <div style={{ flex: 1 }}>
                <div style={{ fontSize: 12, color: 'var(--text2)', marginBottom: 4 }}>Colore avatar</div>
                <div style={{ display: 'flex', gap: 5 }}>
                  ...
                  <input type="color" value={avatarColor} onChange={e => onAvatarColorChange(e.target.value)}
                    style={{ width: 20, height: 20, border: 'none', padding: 0, borderRadius: '50%',
            ...
            <div style={{ marginTop: 16, display: 'flex', flexDirection: 'column', gap: 8 }}>
              <button onClick={onExport} className="btn-ghost" style={{ textAlign: 'left' }}>Esporta dati (.json)</button>
              <button onClick={() => fileRef.current?.click()} className="btn-ghost" style={{ textAlign: 'left' }}>Importa dati (.json)</button>
              ...
              <button onClick={onReset} style={{ textAlign: 'left', background: 'transparent',
                ...
                padding: '8px 16px', fontSize: 13, color: 'var(--danger)', cursor: 'pointer' }}>
                Riconfigura orario
              </button>
        ...
        {tab === 'share' && (
          <div>
            <p style={{ fontSize: 13, color: 'var(--text2)', marginBottom: 16, lineHeight: 1.6 }}>
              Genera un link per condividere il tuo orario in modalità sola lettura. Chiunque abbia il link può vederlo senza account.
            </p>
            {!shareToken ? (
              <button className="btn-primary" onClick={onCreateShare} style={{ width: '100%' }}>
              ...
              <div>
                ...
                <div style={{ display: 'flex', gap: 8 }}>
                  <button className="btn-primary" onClick={copyShare} style={{ flex: 2 }}>
                    {copied ? '✓ Copiato!' : 'Copia link'}
                  </button>
                  <button className="btn-danger" onClick={onDeleteShare} style={{ flex: 1 }}>Revoca</button>
                </div>
              </div>
            )}
          </div>
        ...
        {tab === 'hours' && (
          <div>
            <p style={{ fontSize: 13, color: 'var(--text2)', marginBottom: 16 }}>
              Ore eliminate dalla tabella. Puoi ripristinarle qui.
            </p>
            {hiddenHours.length === 0 ? (
              <div className="empty-state">Nessuna ora nascosta.<br />Clicca × sul numero dell'ora nella tabella.</div>
            ) : (
              hiddenHours.sort((a, b) => a - b).map(h => (
                <div key={h} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center',
                  padding: '8px 12px', background: 'var(--bg2)', borderRadius: 4, marginBottom: 6 }}>
                  <span style={{ fontFamily: 'var(--mono)', fontSize: 13 }}>{h}ª ora</span>
                  <button className="btn-ghost" onClick={() => onRestoreHour(h)}
                    style={{ padding: '3px 10px', fontSize: 12 }}>Ripristina</button>
                </div>
              ))
            )}
          </div>
...
export default function TimetablePage({ user, onLogout, theme, onThemeChange, isOnline }) {
  ...
  const handleSwap = async (from, to) => {
    ...
    try {
      ...
      setSlots(res.data);
    } catch (e) { showToast('Errore nello spostamento', 'err'); }
  };
  ...
  const toggleHideHour = async (hour) => {
    const newHidden = hiddenHours.includes(hour)
      ? hiddenHours.filter(h => h !== hour)
      : [...hiddenHours, hour];
  ...
  const handleDeleteShare = async () => {
    if (!window.confirm('Revocare il link? Chi lo possiede non potrà più accedere.')) return;
  ...
  if (loading) return (
    <div style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', height: '100vh' }}>
      <span style={{ fontFamily: 'var(--mono)', fontSize: 11, color: 'var(--text3)', letterSpacing: '0.1em' }}>CARICAMENTO...</span>
    </div>
  );
    ...
    <div style={{ minHeight: '100vh', background: 'var(--bg)' }}>
      ...
      <header style={{ background: 'var(--card)', borderBottom: '1px solid var(--border)',
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
          ...
          {isLocked && <span style={{ fontFamily: 'var(--mono)', fontSize: 10, color: 'var(--warning)' }}>· BLOCCATO</span>}
          {!isOnline && <span style={{ fontFamily: 'var(--mono)', fontSize: 10, color: 'var(--danger)', background: 'var(--danger-bg)',
            padding: '2px 6px', borderRadius: 3 }}>OFFLINE</span>}
        </div>

        <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
          {/* View toggle */}
          <div style={{ display: 'flex', background: 'var(--bg2)', borderRadius: 4,
            border: '1px solid var(--border)', overflow: 'hidden' }}>
            {[{ v: 'week', l: 'Settimana' }, { v: 'today', l: 'Oggi' }].map(opt => (
              <button key={opt.v} onClick={() => setView(opt.v)}
                style={{ padding: '5px 12px', fontSize: 12, fontWeight: 500, border: 'none', borderRadius: 0,
          ...
          <div onClick={() => setShowSettings(true)}
            style={{ width: 30, height: 30, borderRadius: '50%', background: avatarColor,
              display: 'flex', alignItems: 'center', justifyContent: 'center',
              cursor: 'pointer', fontSize: 13, fontWeight: 700, color: 'white',
              flexShrink: 0, title: user?.username }}>
      ...
      {/* Stats strip */}
      <div style={{ borderBottom: '1px solid var(--border)', padding: '8px 20px',
        display: 'flex', gap: 24, background: 'var(--bg2)', overflowX: 'auto' }}>
        {[
          ...
          <div key={s.label} style={{ display: 'flex', alignItems: 'baseline', gap: 5, flexShrink: 0 }}>
            <span style={{ fontFamily: 'var(--mono)', fontSize: 16, fontWeight: 600, color: 'var(--primary)' }}>{s.value}</span>
      ...
      {view === 'today' ? (
        ...
        <div style={{ padding: 16, overflowX: 'auto', paddingBottom: 80 }}>
          <div style={{
            ...
            {hours.map(hour => (
              <>
                {/* Numero ora con × per nascondere */}
                <div key={`h${hour}`}
                  style={{ display: 'flex', alignItems: 'center', justifyContent: 'center',
                    background: 'var(--bg2)', border: '1px solid var(--border)',
                    borderRadius: 4, position: 'relative', cursor: 'default' }}
                  onMouseEnter={e => { const btn = e.currentTarget.querySelector('.hide-hour-btn'); if (btn) btn.style.opacity = '1'; }}
                  onMouseLeave={e => { const btn = e.currentTarget.querySelector('.hide-hour-btn'); if (btn) btn.style.opacity = '0'; }}>
                  <span style={{ fontFamily: 'var(--mono)', fontSize: 11, fontWeight: 600, color: 'var(--text2)' }}>{hour}</span>
                  <button
                    className="hide-hour-btn"
                    onClick={() => toggleHideHour(hour)}
          ...
          <div style={{ marginTop: 16, display: 'flex', gap: 16, justifyContent: 'center',
            flexWrap: 'wrap', color: 'var(--text3)', fontSize: 11 }}>
            <span>Clicca cella per modificarla</span>
            <span>Trascina per spostare</span>
            <span>Hover su "n" per leggere le note</span>
            <span>× sull'ora per nasconderla</span>
          </div>
        </div>
      )}
      ...
      {/* Toast */}
      {toast && (
        <div style={{ position: 'fixed', top: 60, right: 16, zIndex: 300,
