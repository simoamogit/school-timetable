...
router.post('/slots', async (req, res) => {
  ...
  try {
    await pool.query(
      ...
      [req.user.id, day, hour, subject || '', color || '#6366f1']
    );
    res.json({ ok: true });
  } catch (e) { console.error(e); res.status(500).json({ error: 'Errore server' }); }
});
...
router.get('/substitutions', async (req, res) => {
  try {
    const r = await pool.query(
      'SELECT * FROM substitutions WHERE user_id=$1 ORDER BY sub_date DESC', [req.user.id]
    );
    res.json(r.rows);
  } catch (e) { console.error(e); res.status(500).json({ error: 'Errore server' }); }
});
...
router.post('/substitutions', async (req, res) => {
  const { day, hour, substitute, sub_date, note } = req.body;
  if (!substitute || !sub_date) return res.status(400).json({ error: 'Dati mancanti' });
  try {
    const r = await pool.query(
      'INSERT INTO substitutions (user_id, day, hour, substitute, sub_date, note) VALUES ($1,$2,$3,$4,$5,$6) RETURNING id',
      [req.user.id, day, hour, substitute, sub_date, note || '']
    );
    res.json({ id: r.rows[0].id });
  } catch (e) { console.error(e); res.status(500).json({ error: 'Errore server' }); }
});
...
router.get('/export', async (req, res) => {
  try {
    const [sR, slR, nR, subR] = await Promise.all([
      ...
      pool.query('SELECT day, hour, substitute, sub_date, note FROM substitutions WHERE user_id=$1', [req.user.id])
    ]);
    const s = sR.rows[0];
    res.json({
      version: 1,
  ...
  } catch (e) { console.error(e); res.status(500).json({ error: 'Errore export' }); }
});
...
router.post('/import', async (req, res) => {
  ...
  try {
    ...
    for (const n of notes)
      await client.query(
        'INSERT INTO notes (user_id, day, hour, content, note_date) VALUES ($1,$2,$3,$4,$5)',
        [req.user.id, n.day, n.hour, n.content, n.note_date || null]
      );
    for (const s of substitutions)
      await client.query(
        'INSERT INTO substitutions (user_id, day, hour, substitute, sub_date, note) VALUES ($1,$2,$3,$4,$5,$6)',
        [req.user.id, s.day, s.hour, s.substitute, s.sub_date, s.note || '']
      );
